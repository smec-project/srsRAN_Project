        ...
        while self.running:
            try:
                ...
                for rnti in list(self.request_start_times.keys()):
                    ...
                    elapsed_time_ms = (current_time - requests[min(requests.items(), key=lambda x: x[1])[0]]) * 1000
                    ...
                    self.log_file.write(f"Updated priority for RNTI {rnti} to {priority} at {time.time()}\n")
                    self.log_file.flush()
